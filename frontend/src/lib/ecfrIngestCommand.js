 ...
 *
 * Parses args, POSTs to /api/regulations/sources/ecfr, then polls the sources
 * listing every POLL_MS until the matching (cfr_title, cfr_part) row is
 * `ready` or `failed`. All progress is emitted as BrainScanner log entries
 * via the provided `pushLog({ ts, level, logger, msg })` callback.
 */
...
export async function runEcfrIngestCommand(rawCmd, pushLog) {
  ...
  while (Date.now() < deadline) {
    ...
    if (row.status !== lastStatus) {
      pushLog({
        ts: now(), level: 'INFO', logger: LOGGER,
        msg: `status=${row.status} chunks=${row.chunk_count ?? 0}`,
      })
