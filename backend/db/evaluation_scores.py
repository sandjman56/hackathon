...
def upsert_score(
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            ...
            ON CONFLICT (project_id, evaluation_id) DO UPDATE SET
...
def get_score(conn: Any, project_id: int, evaluation_id: int) -> Optional[dict]:
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            "SELECT * FROM evaluation_scores "
            "WHERE project_id = %s AND evaluation_id = %s",
            (project_id, evaluation_id),
        )
        row = cur.fetchone()
    if not row:
